...
//M: In TargetFunction, what are the References and Specifier for????

public class GoalModule : Module {
	//M: List of keyframes
	public bool[] Keys = new bool[0];
	//M: Type of the goal: DynamicRoot, StaticObject, MovingObject
	public TargetFunction Target = null;
	//M: Action at goal
	public GoalFunction[] Functions = new GoalFunction[0];

	//M:
	public Frame GoalFrame = null;

	public Frame GetGoalFrame()
    {
		ContactModule contactModule = (ContactModule)Data.GetModule(Module.ID.Contact);
		ContactModule.Sensor hipContactSensor = contactModule.GetSensor("pelvis");
		for (int i = 1; i < Data.GetTotalFrames(); i++)
        {
			Frame curr = Data.Frames[i];
			if (hipContactSensor.GetContact(curr, false) == 1f)
            {
				return Data.Frames[i+30];
            }
	...
	public void SetGoalFrame(int val)
	{
		GoalFrame = Data.Frames[val-1];
	...
	public override Module Initialise(MotionData data) {
		...
		//M
		GoalFrame = GetGoalFrame(); 

		//M: SMPLx initialization
		AddGoal("Idle");
		AddGoal("Walk");
		AddGoal("Run");
		AddGoal("Sit");
		AddGoal("Liedown");
		GoalFunction Idlefn = GetGoalFunction("Idle");
		Idlefn.Toggle(Data.Frames.First());
		Target.SetPivot(Data.Frames.First(), TargetFunction.Pivot.DynamicRoot);
		Idlefn.Toggle(Data.Frames.Last());
		Target.SetPivot(Data.Frames.Last(), TargetFunction.Pivot.DynamicRoot);
		//LieDownAutoLabel();
		Inspect = true;
	...
	public void InsertWalkBreak(Frame curr)
    {
		ToggleKey(curr);
		Frame prevKeyFrame = GetPreviousKey(curr);
		GoalFunction IdleGaolFn = GetGoalFunction("Idle");
		GoalFunction WalkGoalFn = GetGoalFunction("Walk");
		GoalFunction SitGoalFn = GetGoalFunction("Sit");
		SitGoalFn.Toggle(prevKeyFrame);
		WalkGoalFn.Toggle(prevKeyFrame);
		Target.SetPivot(prevKeyFrame, TargetFunction.Pivot.DynamicRoot);
		Target.SetProp(prevKeyFrame, string.Empty);
		Target.SetSpecifier(prevKeyFrame, string.Empty);

	}

	public void SitAutoLabel()
	{
		GoalFunction IdleGaolFn = GetGoalFunction("Idle");
		GoalFunction WalkGoalFn = GetGoalFunction("Walk");
		GoalFunction SitGoalFn = GetGoalFunction("Sit");
		...
		bool sitHappended = false;
		Frame prevKeyFrame = Data.Frames[0];

		string objectName = Data.GetScene().GetRootGameObjects()[0].transform.GetChild(0).name;

		while (true)
        {
			Frame currKeyFrame = styleModule.GetNextKey(prevKeyFrame);
			if (currKeyFrame == Data.Frames.Last())
			{ break; }
			//M: character start moving towards the target
			if (styleModule.GetStyle(currKeyFrame, "Idle") == 1 && styleModule.GetStyle(prevKeyFrame, "Idle") == 1)
            {
				ToggleKey(currKeyFrame);
                IdleGaolFn.Toggle(currKeyFrame);
                SitGoalFn.Toggle(currKeyFrame);
                Target.SetPivot(currKeyFrame, TargetFunction.Pivot.GoalFrame);
                Target.SetProp(currKeyFrame, objectName);
                //Target.SetSpecifier(currKeyFrame, "right_hip");

            }

			//M: character has achieved the main action "Sit" and is starting to move away from the target
			else if (styleModule.GetStyle(currKeyFrame, "Sit") == 1 && styleModule.GetStyle(prevKeyFrame, "Sit") == 1)
			{
				ToggleKey(currKeyFrame);
                SitGoalFn.Toggle(currKeyFrame);
                WalkGoalFn.Toggle(currKeyFrame);
                Target.SetPivot(currKeyFrame, TargetFunction.Pivot.DynamicRoot);
                //M: Is this important?
                //Target.SetProp(currKeyFrame, objectName);
                Target.SetSpecifier(currKeyFrame, string.Empty);
                sitHappended = true;
            }

			//M: character has moved away from the target and now it will start normal walking
			else if (sitHappended && styleModule.GetStyle(currKeyFrame, "Walk") == 1 && styleModule.GetStyle(prevKeyFrame, "Sit") == 1)
			{
				ToggleKey(currKeyFrame);
				Target.SetProp(currKeyFrame, string.Empty);
				Target.SetPivot(currKeyFrame, TargetFunction.Pivot.DynamicRoot);
			}

			//M: character is going to idle mode
			else if (sitHappended && styleModule.GetStyle(currKeyFrame, "Walk") == 1 && styleModule.GetStyle(prevKeyFrame, "Walk") == 1)
			{
				ToggleKey(currKeyFrame);
				WalkGoalFn.Toggle(currKeyFrame);
				IdleGaolFn.Toggle(currKeyFrame);
	...
	public void LieDownAutoLabel()
	{
		GoalFunction IdleGaolFn = GetGoalFunction("Idle");
		GoalFunction WalkGoalFn = GetGoalFunction("Walk");
		GoalFunction LiedownGoalFn = GetGoalFunction("Liedown");

		StyleModule styleModule = (StyleModule)Data.GetModule(Module.ID.Style);

		bool LiedownHappended = false;
		Frame prevKeyFrame = Data.Frames[0];
		...
		while (true)
		{
			...
			if (styleModule.GetStyle(currKeyFrame, "Idle") == 1 && styleModule.GetStyle(prevKeyFrame, "Idle") == 1)
			{
				ToggleKey(currKeyFrame);
				IdleGaolFn.Toggle(currKeyFrame);
				LiedownGoalFn.Toggle(currKeyFrame);
				Target.SetPivot(currKeyFrame, TargetFunction.Pivot.GoalFrame);
				Target.SetProp(currKeyFrame, objectName);
				//Target.SetSpecifier(currKeyFrame, "right_hip");
			...
			//M: character has achieved the main action "Sit" and is starting to move away from the target
			else if (styleModule.GetStyle(currKeyFrame, "Liedown") == 1 && styleModule.GetStyle(prevKeyFrame, "Liedown") == 1)
			{
				ToggleKey(currKeyFrame);
				LiedownGoalFn.Toggle(currKeyFrame);
				WalkGoalFn.Toggle(currKeyFrame);
				...
				//Target.SetProp(currKeyFrame, objectName);
				Target.SetSpecifier(currKeyFrame, string.Empty);
				LiedownHappended = true;
			}

			//M: character has moved away from the target and now it will start normal walking
			else if (LiedownHappended && styleModule.GetStyle(currKeyFrame, "Walk") == 1 && styleModule.GetStyle(prevKeyFrame, "Liedown") == 1)
			{
				ToggleKey(currKeyFrame);
				Target.SetProp(currKeyFrame, string.Empty);
				Target.SetPivot(currKeyFrame, TargetFunction.Pivot.DynamicRoot);
	...
	public float GetAction(Frame frame, string name)
	{
		GoalFunction function = GetGoalFunction(name);
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		if (Utility.GUIButton("InsertWalkBreak", IsKey(frame) ? UltiDraw.Cyan : UltiDraw.DarkGrey, IsKey(frame) ? UltiDraw.Black : UltiDraw.White))
		{
			InsertWalkBreak(frame);
		}

		//SetGoalFrame(EditorGUILayout.IntField("GoalFrame", GoalFrame.Index));
	...
	public class TargetFunction {
		//M: DynamicRoot is the default setting and covers walking and idle motion
		//M: StaticObject : Goal object is static (e.g. going towards a box or a chair)
		//M: DynamicObject : Goal object is dynamic/moving (e.g. the box is being moved by the character)
		public enum Pivot {DynamicRoot, StaticRoot, DynamicObject, StaticObject, GoalFrame};
		...
		public Pivot[] Pivots;
		public int[] References;//M: The usage of this is not clear
		public string[] Props; //M: Name of the object
		public string[] Specifiers;//M: Contact points in goal gameobject
		...
		public TargetFunction(GoalModule module) {
			...
			for(int i=0; i<References.Length; i++) {
				References[i] = (int)Module.Data.Framerate;
			}
		...
		public Vector3 GetGoalPosition(Frame reference, float offset, bool mirrored, float delta) {
			...
			//M
			else if (GetPivot(frame) == Pivot.GoalFrame)
			{
				return Module.GoalFrame.GetBoneTransformation(0, mirrored).GetPosition();
			}

			else {
				//M: Goal is computed based on the root motion in this case ( walking and idle)
		...
		public Quaternion GetGoalRotation(Frame reference, float offset, bool mirrored, float delta) {
			...
			{
				RootModule module = Module.Data.GetModule(ID.Root) == null ? null : (RootModule)Module.Data.GetModule(ID.Root);
				Matrix4x4 root = module.GetRootTransformation(Module.GoalFrame, mirrored);
	...
	public class GoalFunction {
		...
		//M: Fill values between key frames?
		public void Compute(Frame frame) {
			Frame current = frame;
			Frame previous = Module.GetPreviousKey(current);
			Frame next = Module.GetNextKey(current);
