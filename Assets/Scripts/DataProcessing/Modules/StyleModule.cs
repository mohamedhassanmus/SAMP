...
public class StyleModule : Module {
	...
	//M: List of all styles available in the data. e.g: idle, walk, sit. Each one is and object of type StyleFunction.
	// The StyleFunction holds the numuric value for this specific style (eg. Idle) for all frames. It also holds the name of the style
	...
	public override Module Initialise(MotionData data) {
		...
		//M: Make first and last frames keyFrames
		Keys[0] = true;
		...
		AddStyle("Sit");
		AddStyle("Liedown");
		StyleFunction Idlefn = GetStyleFunction("Idle");
	...
	public int ComputeNumFramesInStyle(string name)
	{
		int count = 0;
		StyleFunction fn = GetStyleFunction(name);
		if (fn != null)
        {
			for (int i = 0; i < fn.Values.Length; i++)
			{
				if (fn.Values[i] > 0.5) { count++; }
			}
		}
    ...
    {
		StyleFunction Idlefn = GetStyleFunction("Idle");
		StyleFunction Walkfn = GetStyleFunction("Walk");
		StyleFunction Sitfn = GetStyleFunction("Sit");

		ContactModule contactModule = (ContactModule)Data.GetModule(Module.ID.Contact);
		ContactModule.Sensor right_ankle = contactModule.GetSensor("right_ankle");
		ContactModule.Sensor left_ankle = contactModule.GetSensor("left_ankle");


		ContactModule.Sensor hipContactSensor = contactModule.GetSensor("pelvis");


		Frame prev = Data.Frames[0];
		Vector3 inithip = prev.GetBoneTransformation("pelvis", false).GetPosition();
		float hipTH = 0.05f;
		float hipVelocityTH1 = 0.2f;
		float hipVelocityTH2 = 0.001f;

		bool detectIdle = false;

		string prevStyle = "Idle";
        for (int i = 1; i < Data.GetTotalFrames(); i++)
        {
	
			Frame curr = Data.Frames[i];
			Vector3 hip = curr.GetBoneTransformation("pelvis", false).GetPosition();
			float hipVelocity = curr.GetBoneVelocity("pelvis", false, 1/Data.Framerate).sqrMagnitude;


			/* Idle2Walk. Based on one complete walk cycle. Not robust */
			// right foot is not in contact -> trasition to walking
            if (prevStyle == "Idle" && right_ankle.GetContact(curr, false) != 1f)
            {
				Keys[i] = true;
				prevStyle = "right_up";
			}

			else if (prevStyle == "Idle" && left_ankle.GetContact(curr, false) != 1f)
			{
				Keys[i] = true;
				prevStyle = "left_up";
			}

			// right foot back on the floor -> walking started, end of idling
			else if (prevStyle == "right_up" && right_ankle.GetContact(curr, false) == 1f && left_ankle.GetContact(curr, false) != 1f)
			{
				
				prevStyle = "right_down_left_up";
				
			}

			// right foot back on the floor -> walking started, end of idling
			else if (prevStyle == "left_up" && left_ankle.GetContact(curr, false) == 1f && right_ankle.GetContact(curr, false) != 1f)
			{

				prevStyle = "left_down_right_up";

			}

			// right foot back on the floor -> walking started, end of idling
			else if ( (prevStyle == "right_down_left_up" && left_ankle.GetContact(curr, false) == 1f) || (prevStyle == "left_down_right_up" && right_ankle.GetContact(curr, false) == 1f))
			{
				Keys[i] = true;
				Idlefn.SetValue(curr, 0f);
				Walkfn.SetValue(curr, 1f);
				//Idlefn.Toggle(curr);
				//Walkfn.Toggle(curr);
				prevStyle = "Walk";
				// This might be a better guess compared to the init T-pose
				inithip = prev.GetBoneTransformation("pelvis", false).GetPosition();
			}


			/* Walk2Sit. Based on one complete walk cycle. Not robust */
			//hip moving downwards -> transit to sitting
			else if (prevStyle == "Walk" && Mathf.Abs(hip.y - inithip.y) > hipTH)
			{
				var tmp = i - 30;
				Keys[tmp] = true;
				Idlefn.SetValue(Data.Frames[tmp], 0f);
				Walkfn.SetValue(Data.Frames[tmp], 1f);
				prevStyle = "Walk2Sit";
			}

			//hip touch the chair -> sittig started
			else if (prevStyle == "Walk2Sit" && hipContactSensor.GetContact(curr, false) == 1f)
			{
				Keys[i] = true;
				Idlefn.SetValue(curr, 0f);
				Walkfn.SetValue(curr, 0f);
				Sitfn.SetValue(curr, 1f);
				prevStyle = "Sit";
			}

			//hip not in contact with the chair -> trasit to walking
			else if (prevStyle == "Sit" && hipContactSensor.GetContact(curr, false) != 1f)
			{
				// Shift to ensure that we capture transition from any style to relax style to stand up
				var tmp = i - 50;
				Keys[tmp] = true;
				Idlefn.SetValue(Data.Frames[tmp], 0f);
				Walkfn.SetValue(Data.Frames[tmp], 0f);
				Sitfn.SetValue(Data.Frames[tmp], 1f);
				prevStyle = "Sit2walk";
			}

            // character is fully up and started walking
            else if (prevStyle == "Sit2walk" && Mathf.Abs(hip.y - inithip.y) < hipTH)
			{
				i += 15;
				var tmp = i;
				Keys[tmp] = true;
				Idlefn.SetValue(Data.Frames[tmp], 0f);
				Walkfn.SetValue(Data.Frames[tmp], 1f);
				Sitfn.SetValue(Data.Frames[tmp], 0f);
				prevStyle = "Walk";
				detectIdle = true;
			}

        //else if (detectIdle && prevStyle == "Walk" && i == Data.GetTotalFrames() - 30)
        //{
        //	Keys[i] = true;
        //	Idlefn.SetValue(curr, 0f);
        //	Walkfn.SetValue(curr, 1f);
        ...
        //else if (detectIdle && prevStyle == "Walk2Idle" && i == Data.GetTotalFrames() - 20)
        //{
        //	Keys[i] = true;
        //	Idlefn.SetValue(curr, 1f);
        //	Walkfn.SetValue(curr, 0f);
        //	break;
        //}

        //M: deteting idle state with velocity seems noisy

            else if (detectIdle && i > (Data.GetTotalFrames() - 100) && prevStyle == "Walk" && hipVelocity < hipVelocityTH1)
            {
                Keys[i] = true;
                Idlefn.SetValue(curr, 0f);
                Walkfn.SetValue(curr, 1f);
            ...
            else if (detectIdle && i > (Data.GetTotalFrames() - 100) && prevStyle == "Walk2Idle" && hipVelocity < hipVelocityTH2)
            {
                Keys[i] = true;
                Idlefn.SetValue(curr, 1f);
                Walkfn.SetValue(curr, 0f);
	...
	public void LieDownAutoLabel()
	{
		StyleFunction Idlefn = GetStyleFunction("Idle");
		StyleFunction Walkfn = GetStyleFunction("Walk");
		StyleFunction LieDownfn = GetStyleFunction("Liedown");
		StyleFunction Sitfn = GetStyleFunction("Sit");
		...
		for (int i = 1; i < Data.GetTotalFrames(); i++)
		{
			...
			else if (prevStyle == "Walk2Sit" && hipContactSensor.GetContact(curr, false) == 1f)
			{
				...
				Walkfn.SetValue(Data.Frames[tmp], 0f);
				Sitfn.SetValue(Data.Frames[tmp], 0f);
				LieDownfn.SetValue(Data.Frames[tmp], 1f);
				prevStyle = "Sit";
			}
			...
			else if (prevStyle == "Sit" && hipContactSensor.GetContact(curr, false) != 1f)
			{
				...
				Walkfn.SetValue(Data.Frames[tmp], 0f);
				Sitfn.SetValue(Data.Frames[tmp], 1f);
				LieDownfn.SetValue(Data.Frames[tmp], 0f);
